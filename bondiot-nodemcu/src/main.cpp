...
#include "PubSubClient.h"
#include "ArduinoJson.h"
#include "libsniffer_functions.h"
...
void loop() {
  ...
  while (channel < 15) {                    // Only scan channels 1 to 14
	...
	if (readingLoadCell != 0){		
		if (abs(readingLoadCell-last_weight) >= weight_variation){
			//add to json
		}			
	...
	//create json
	//una vez que se envian los datos se RESETEAN las variables, ejemplo cant_pasajeros_subiendo
	//falta
